...
class Leaflet:
    ...
    def __init__(self, projection, **kwargs):
        ...
        if kwargs["draw_control"]:
            ...
            if "marker" in kwargs["draw_tools"]:
                draw_control.marker = dict(shapeOptions=shapeOptions)
            # add polyline tool
            if "polyline" in kwargs["draw_tools"]:
                draw_control.polyline = dict(shapeOptions=shapeOptions)
            # add rectangle tool
            if "rectangle" in kwargs["draw_tools"]:
                draw_control.rectangle = dict(
                    shapeOptions=shapeOptions, metric=["km", "m"]
                )
            # add polygon tool
    ...
    def wrap_longitudes(self, lon):
        ...
        phi = np.arctan2(
            np.sin(lon * np.pi / 180.0), np.cos(lon * np.pi / 180.0)
        )
    ...
    def plot_geometries(self, ax=None, **kwargs):
        ...
        gdf = gpd.GeoDataFrame.from_features(
            self.geometries, crs=self.geometries["crs"]
        ).to_crs(self.crs)
...
class LeafletMap(HasTraits):
    ...
    def plot(self, m, **kwargs):
        ...
        if kwargs["norm"] is None:
            self.norm = colors.Normalize(
                vmin=self.vmin, vmax=self.vmax, clip=True
            )
        else:
    ...
    def get_bbox(self):
        ...
        (self.left, self.top), (self.right, self.bottom) = self.map.pixel_bounds
        self.sw = dict(
            x=(self.map.crs["origin"][0] + self.left * self.resolution),
            y=(self.map.crs["origin"][1] - self.bottom * self.resolution),
        )
        self.ne = dict(
            x=(self.map.crs["origin"][0] + self.right * self.resolution),
    ...
    def get_norm_bounds(self, **kwargs):
        ...
        clim = (
            self._ds_selected.chunk(dict(y=-1, x=-1))
            .quantile((0.02, 0.98))
            .values
        )
    ...
    def clip_image(self, ds):
        ...
        if resolution > self.resolution:
            ...
            dst_transform = rio.transform.from_origin(
                minx, maxy, self.resolution, self.resolution
            )
    ...
    def set_dataset(self):
        ...
        """
        # reduce to variable and lag
        if (self._ds[self._variable].ndim == 3) and (
            "time" in self._ds[self._variable].dims
        ):
            self._ds_selected = self._ds[self._variable].sel(
                time=self._ds.time[self.lag]
            )
        elif (self._ds[self._variable].ndim == 3) and (
            "band" in self._ds[self._variable].dims
        ):
    ...
    def handle_click(self, **kwargs):
        ...
        self._popup = ipyleaflet.Popup(
            location=(lat, lon), child=child, name="popup"
        )
    ...
    def add_colorbar(self, **kwargs):
        ...
        self._colorbar = ipyleaflet.WidgetControl(
            widget=output, transparent_bg=False, position=kwargs["position"]
        )
        # add colorbar
    ...
    def imshow(self, ax=None, **kwargs):
        ...
        # extract units
        longname = re.sub(
            r"\s+", " ", self._ds[self._variable].attrs["long_name"]
        )
...
class TimeSeries(HasTraits):
    ...
    def point(self, ax, fields=[], **kwargs):
        ...
        # add legend
        if add_legend:
            label = "{0:8.4f}\u00b0N, {1:8.4f}\u00b0E".format(lat, lon)
    ...
    def transect(self, ax, fields=[], all_touched=False, **kwargs):
        ...
        for i, t in enumerate(self._ds.time):
            ...
            (self._line[i],) = ax.plot(
                self._dist, self._data[:, i], label=labels[i], **kwargs
            )
        # set labels and title
